...
G_SYNOPSIS="
 ...
 DESCRIPTION
        ...
        'philips_diffusionProcess.bash' performs processing on diffusion
	sequences captured on Philips machines. It's main purpose is to 
 ...
 STAGES
	...
	is performed.
		...
		1	-	Collect the Philips diffusion sequence.
...
# Error messages
EM_manufacturer="this doesn't seem to be a Philips sequence."
...
if (( !Gb_useOverrideOut )) ; then
	G_OUTDIR=$G_DICOMINPUTDIR/PHILIPS_DIFFUSION
else
...
declare -i b_PHILIPS=0
b_PHILIPS=$(echo "$MANUFACTURER" | grep -i Philips | wc -l)
if (( !b_PHILIPS )) ; then fatal manufacturer ; fi
...
if (( ${barr_stage[2]} )) ; then
    ...
    if (( ! ${#BVAL}  || ! ${#BVEC} )) ; then 
        # Sometimes with Philips scans, dcm2nii inserts 'x' before
        # filenames for bval/bvec, if so rest
        BVAL=$(ls -1 x${DCM2NIINAME}.bval | head -n 1)
        BVEC=$(ls -1 x${DCM2NIINAME}.bvec | head -n 1)
        
        if (( ! ${#BVAL}  || ! ${#BVEC} )) ; then 
            fatal bvConvert; 
        else
            mv x${DCM2NIINAME}.bval ${DCM2NIINAME}.bval
            mv x${DCM2NIINAME}.bvec ${DCM2NIINAME}.bvec
        fi
    fi
