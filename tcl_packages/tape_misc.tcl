...
#
# o	Added error checking to tape_admin_close - largely redundant, but at 
#	least it's a deeper level of error checking.
#
# o	Basic backup behaviour redefined -
# 
#	Each backup operation is preceded by a `rewind' command. This allows 
#	error checking routines to ascertain whether or not a tape is actually 
#	present in the remote device.
#
#	Additionally, this implies that each backup - daily/weekly/monthly is 
#	performed on its own tape. Of course, the first daily backup after a 
#	weekly or monthly will by necessity have less incremental data 
#	than the backup for several days later, implying an inefficient 
#	use of tape space. However, this should be weighed against the 
#	ease of program operation and is considered to be a worthwhile 
#	trade-off (i.e. using many tapes for simplicity, and having
#	each incremental increase on an own tape).
#
...
#
# 6-29-1998
# o	Correct behaviour when target host is dead... non-fatal error should 
#	terminate host-related backups, but not the entire process.
#
# 7-1-1998
...
#
# 7-13-1998
# o	Began implementing code for cycling incremental records of non-monthly
#	archive sets.
#
# 7-21-1998
# o	Added `archiveDate' to class structure. This records the date of the last
#	successful archive for current set, and is used primarily to determine when
#	a forced delete of an incremental record set is required. For sets that have
#	only a daily and/or weekly backup defined, incremental backups are
#	made relative to the first time that the set is processed. The `archiveDate'
#	is used to determine when this `base' should be erased.
#
# 7-28-1998
...
#
# This routine simply returns a list defining the records of the tape class
#
    set classStruct {
	name 
	archiveDate
...
# action		in	action being performed when error occurred
# errorCondition	in	error message caught
# exitcode	in	internal error number (sent to system on shutdown)
# type		in (opt)	error type - if "fatal" shutdown, else continue
#
    upvar $class tape
    global ext
...
#
# Simply checks whether or not today's date falls within the first week (7 days)
# of the month
#
...
# rule		in	target rule
# ruleDays	returned	number of days and list of days with target rule
#
    upvar $class tape
...
proc tape_notice_sendMail {class subject {bodyFile "void"}} {
#
# Sends a mail message to a tape's adminUser
#
# A mail message containing the `subject' is sent to a target tape's adminUser.
# If an optional bodyFile is sent, the contents of this file are mailed to the
# adminUser as the message's body.
#
...
#
# Before implementing a backup, some general admin functions are performed,
# specifically the deletion of files containing old backup status and info. 
# New empty files are created which will contain result and status 
# information
#
# The proper data path for the working directory is read from class definition
#
...
#
# The results of an archive process (the path/files that are returned from the 
# remote backup.tcl process) are written to a results file. Additionally, these
# results are parsed and some status information is also extracted and 
...
#
# Increment the current rule's set number for class
#
...
# this manager process. On being called, the `notifyTape' field of the target
# class is executed on the class's remote host.
# 
...
#
    ...
    catch "exec $tape(rsh) -l $tape(remoteUser) $tape(remoteHost) mt -f $tape(remoteDevice) \
	$tape(command)" result

    if {([lsearch $result "error"] >= 0) || ([lsearch $result mt] >= 0)} {
	set status "failed"
	puts "${status}."
...
# class		in	current tape being processed
# date		in	target date
# silent		in (opt)	optional verbose flag. If set, don't echo output	
...
#
# class		in	current tape being processed
# backup_done	returned	status of backup set. If `1' all backups in current set
#			completed successfully. If `0', one backup in current
#			set failed (typically if remote host did not respond)
...
#
	...
	set tape(command) "/sbin/ping -c 3 $host"
	...
	puts -nonewline "\n\t$label - "
	set tape(command) "$tape(rsh) $host /root/arch/scripts/backup.tcl "
	append tape(command) "-u $tape(remoteUser) "
	append tape(command) "-h $tape(remoteHost) "
	append tape(command) "-d $tape(remoteDevice) "
	append tape(command) "-l $label "
	append tape(command) "-f $filesys "
	append tape(command) "-c $tape(currentRule) "
	append tape(command) "-b buffer "
	append tape(command) "-r $tape(rsh) "
	append tape(command) "-i $incReset "
	if {$tape(currentRule) != "monthly"} {
	    append tape(command) "-v on" 
	} else {
	    append tape(command) "-v off"
	}
...
#
# class		in	current tape being processed
# forceRule	in (opt)	contains a rule type, forcing operation to
#			default to forceRule
# forceDay	in (opt)	contains a day name, forcing operation to
#			default to forceDay's rule
# tapeInit		in (opt)	list of tape initialisation commands (e.g. rewind)
#
    upvar $class tape
	...
	switch -- [exec uname] {
	    ...
	    default { tomorrowDate [exec date --date '1 day'] }
	}
	...
	puts "********************\n"
	tape_notice_sendMail tape "$message"
    }
    return $backup_status
}
