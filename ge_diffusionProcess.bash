...
b_STAGEDONE=$(ls -1 ${G_OUTDIR}/*1.dcm 2>/dev/null | wc -l)
cprint "Pre-collected DICOMS" "[ $b_STAGEDONE ]"
STAGE1PROC=dicom_seriesCollect.bash
if (( ${barr_stage[1]} && ! $b_STAGEDONE )) ; then
...
STAGE=2-$STAGE2PROC
STAGE2DIR="${G_OUTDIR}"
b_STAGEDONE=$(ls -1 ${G_OUTDIR}/*.nii.gz 2>/dev/null | wc -l)
cprint "Existing *nii.gz" "[ $b_STAGEDONE ]"
if (( ${barr_stage[2]} && ! $b_STAGEDONE )) ; then
